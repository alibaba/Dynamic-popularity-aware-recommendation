...
times = sorted(train_df.time.dt.strftime('%Y-%m-%d').unique())

train_df["dt"] = train_df.time.dt.strftime('%Y-%m-%d')
train_df_dtgroup = train_df.groupby("dt")
params = {
...
for i in range(6):
    for dt in times:
        train_df_tmp = train_df_dtgroup.get_group(dt).sample(frac=1)
        print(dt, len(train_df_tmp))
        dy_steps = int(len(train_df_tmp) / FLAGS.batch_size * 5 + 1)
        model.train(input_fn, steps=dy_steps)
        model.evaluate(eval_input_fn, steps=100)
...
np.save("itemid_vectors_M5.npy", id_vectors)
np.save("itemid_index_M5.npy", id_index)
print("output item vector")
...
id_vectors = np.array(id_vectors)
id_index = np.array(id_index)
np.save("userid_vectors_M5.npy", id_vectors)
np.save("userid_index_M5.npy", id_index)
